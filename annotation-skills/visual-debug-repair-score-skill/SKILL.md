---
name: visual-debug-repair-score-skill
description: Visual Debug single-candidate repair scoring rubric for AIDP VLM Coding new-process tasks. Use when Codex needs to label this queue in Chrome by checking the prompt-described visual bug, repaired webpage output, reference image fidelity, abandon rules, 0/1/2 scoring, and the rule that only one best candidate can receive 2 points.
---

# Visual Debug Repair Score Skill

## Goal

Use the bundled Visual Debug manual rules to score repaired webpage outputs. Prioritize task-specific repair completion, reference-image fidelity, abandon rules, and the single-best 2-point rule over generic annotation habits.
...
5. Check abandon conditions before normal 0/1/2 scoring.
6. Open the repaired webpage output, scan from first screen to the bottom, and verify load state, repair completion, reference-image fidelity, and UP instruction completion.
7. Compare observations against the current-item checklist one point at a time, then apply 0/1/2 scoring and the single-best 2-point rule when multiple candidates are present.
...
5. Scan the repaired webpage from first screen to bottom and test only prompt-named controls if they affect the requested repair.
