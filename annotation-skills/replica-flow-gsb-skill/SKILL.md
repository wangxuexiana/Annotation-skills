...
- `references/user-style.md` for active reason wording constraints. Treat explicit user wording preferences there as hard checks, not optional style examples.
...
2. Convert the current task prompt into a 3-8 point current-item checklist using `references/decision-checklist.md`.
...
6. Draft the reason using the active `references/user-style.md` wording constraints before filling it.
7. Run `references/pre-submit-audit.md` before filling the final label or reason.
8. Pause if a rule conflict, hidden permission gate, login challenge, CAPTCHA, or destructive action appears.
...
1. If `references/user-style.md` contains active wording preferences from the user, follow them as hard constraints.
...
Hard wording constraint: if `references/user-style.md` says the user only wants commas in comparison reasons, enforce that before filling the reason. Rewrite any reason containing other punctuation marks.
